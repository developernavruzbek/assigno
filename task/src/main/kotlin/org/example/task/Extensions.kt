...
fun userId(): Long {
    return getUserJwtPrincipal()
        ?.claims
        ?.get(USER_ID_KEY) as? Long
        ?: throw IllegalStateException("USER_ID_KEY not found or not Long")
}
fun currentOrgId(): Long? {
    return getUserJwtPrincipal()?.claims?.get(JWT_CURRENT_ORG_ID_KEY)?.toString()?.toLongOrNull()
}
